...
public class EspectaculoDto implements Serializable {
    ...
    public static Espectaculo toEspectaculo(EspectaculoDto item) {
        if (item == null)
            return null;
        return new Espectaculo(item.getPlataforma(), item.getNombre(), item.getDescripcion(), item.getDuracion(), item.getMin_espectador(), item.getMax_espectador(), item.getUrl(), item.getCosto(), new Date(item.getFecha_registro()), item.getId(), item.getId_artista(), logica.enums.EstadoEspectaculo.valueOf(item.getEstado().toString())); // DEBUG, cambiar EstadoEspectaculo
    }
